...
class PoetryDataSet(Dataset):
    def __init__(self, poet_file, max_len=None):
        ...
        self.word_to_ix = np.load(poet_file, allow_pickle=True)[
            'word2ix'].item()
        self.ix_to_word = np.load(poet_file, allow_pickle=True)[
            'ix2word'].item()
...
class RNN(nn.Module):
    def __init__(self, vocab_size: int, hidden_size: int, num_layers: int, **kwargs) -> None:
    ...
    def init_state(self, batch_size, device=torch.device('cpu')):
        return torch.randn((self.num_directions * self.rnn.num_layers, batch_size, self.rnn.hidden_size), device=device)
...
class GRU(nn.Module):
    ...
    def init_state(self, batch_size, device=torch.device('cpu')):
        return torch.randn((self.num_directions * self.gru.num_layers, batch_size, self.gru.hidden_size), device=device)
...
class LSTM(nn.Module):
    ...
    def init_state(self, batch_size, device=torch.device('cpu')):
        return (torch.randn((self.num_directions * self.lstm.num_layers, batch_size, self.lstm.hidden_size), device=device),
                torch.randn((self.num_directions * self.lstm.num_layers, batch_size, self.lstm.hidden_size), device=device))


class peephole_LSTM(nn.Module):
    def __init__(self, vocab_size: int, hidden_size: int, num_layers=1, **kwargs) -> None:
    ...
    def forward(self, input_data, hidden_state):
        ...
        all_hidden_state = torch.empty((0, input_data.shape[1], self.hidden_size), device=input_data.device)
        for time_t_input in input_data:
            self.forgetgate = torch.sigmoid(self.input2forgetgate(time_t_input) + self.hidden2forgetgate(hidden_state) + self.cell2forgetgate(cell_state))
            self.inputgate = torch.sigmoid(self.input2inputgate(time_t_input) + self.hidden2inputgate(hidden_state) + self.cell2inputgate(cell_state))
            self.outputgate = torch.sigmoid(self.input2outputgate(time_t_input) + self.hidden2outputgate(hidden_state) + self.cell2outputgate(cell_state))
            candidate = torch.tanh(self.input2candidate(time_t_input) + self.hidden2candidate(hidden_state) + self.cell2candidate(cell_state))
            cell_state = self.forgetgate * cell_state + self.inputgate * candidate
            hidden_state = self.outputgate * torch.tanh(cell_state)
            all_hidden_state = torch.cat((all_hidden_state, hidden_state.unsqueeze(0)), dim=0)
...
class poetry_model():
    def __init__(self, model, lr=1e-3, epochs=100, loss_func=nn.CrossEntropyLoss, optim=torch.optim.Adam, clipping_theta=1, init_hidden=True, log_dir='logs', device=torch.device('cpu')):
        self.device = device
        self.epochs = epochs
    ...
    def _save_data(self, version: int, model_dir=''):
        if model_dir:
            torch.save(self.model.state_dict(), '%s/model_%d.pth' % (model_dir, version))
            print('[INFO] Saving to %s_%s.pth' % (model_dir, version))

    def _load_data(self, version, model_dir=''):
        if model_dir:
            self.model.load_state_dict(torch.load('%s/model_%d.pth' % (
                model_dir, version), map_location=self.device))
            print('[INFO] Loading from %s_%s.pth' % (model_dir, version))

    def train(self, train_loader, model_dir='model', model_version=0, save_frequency=100):
        if model_version:
        ...
        for epoch in trange(self.epochs, desc='Epoch', file=sys.stdout, leave=False):
            ...
            for vocabulary in t:
                ...
                if not hidden_state or self.init_hidden:
                    hidden_state = self.model.init_state(vocabulary.size(0), self.device)
                elif isinstance(hidden_state, tuple):
                    hidden_state = tuple([h.detach_() for h in hidden_state])
                else:
                    hidden_state.detach_()
                # 准备数据
                vocabulary = vocabulary.long().transpose(1, 0).contiguous().to(self.device)
                ...
                t.set_description('Epoch %d, Loss: %.4f' %
                                  (epoch + 1 + model_version, loss.item()))
            ...
            # 保存loss
            self.writer.add_scalar('Loss', np.mean(loss_list), epoch + 1 + model_version)

        if model_version:
            self.epochs += model_version

    def generate_poerty(self, start_words: str, dataset: PoetryDataSet, max_gen_len=200, model_dir='model', model_version=0) -> str:
        if model_dir:
        ...
        while generated_poetry[-1] != '<EOP>' and len(generated_poetry) < max_gen_len:
            output, state = self.model(torch.tensor(dataset.word_to_ix[generated_poetry[-1]], device=self.device).reshape(-1, 1), state)
            generated_poetry.append(dataset.ix_to_word[torch.argmax(output[0, 0]).item()])
        # 格式化
    ...
    def generate_acrostic_poetry(self, start_words, dataset: PoetryDataSet, max_gen_len=200, model_dir='model', model_version=0):
        if model_dir:
            assert model_version
            self._load_data(version=model_version, model_dir=model_dir)
        ...
        for each_word in start_words_list:
            ...
            while not a_sentence[-1] == '。' and not a_sentence[-1] == '！' and not a_sentence[-1] == '<EOP>' and len(a_sentence) < max_gen_len:
                output, state = self.model(torch.tensor(dataset.word_to_ix[a_sentence[-1]], device=self.device).reshape(-1, 1), state)
...
if __name__ == '__main__':
    ...
    poetry_dataset = PoetryDataSet(poet_file='data/Poetry_data_word2ix_ix2word.npz', max_len=311600)
    train_loader = DataLoader(poetry_dataset, batch_size=400, shuffle=True, num_workers=8)

    # 定义模型
    RNN_model = RNN(vocab_size=poetry_dataset.vocab_size(), hidden_size=256, num_layers=1)
    GRU_model = GRU(vocab_size=poetry_dataset.vocab_size(), hidden_size=256, num_layers=2)
    LSTM_model = LSTM(vocab_size=poetry_dataset.vocab_size(), hidden_size=512, num_layers=1)
    peephole_LSTM_model = peephole_LSTM(vocab_size=poetry_dataset.vocab_size(), hidden_size=512, num_layers=2)
    ...
    # 测试生成藏头诗
    start_words = "深度学习"
    acrostic_poetry = model.generate_acrostic_poetry(start_words, poetry_dataset, model_dir="model", model_version=100)
    print(acrostic_poetry)

    # 测试普通生成诗词
    start_words = "夕阳无限好"
    poetry = model.generate_poerty(start_words, poetry_dataset, model_dir="model", model_version=420)
    print(poetry)
